...
async function executeBulkSync() {
  ...
  try {
    ...
    triggerBilingualNotification(`Failed to aggregate database: ${err.message}`);
  } finally {
    for (const col of collectionsToFetch) {
...
export function subscribeToClinicalData<T>(
  ...
  const loadAndNotify = async (forceRefetch = false) => {
    ...
    try {
      ...
      triggerBilingualNotification(`Failed to load: ${err.message}`);
      onError(err);
    }
  };
...
export async function saveDataPermanently<T extends { id: string }>(
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      attempt++;
      if (attempt >= maxAttempts) {
        triggerBilingualNotification(`Transmission Error [PostgreSQL]: ${err.message}`);
...
export async function deleteDataPermanently(
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      attempt++;
      if (attempt >= maxAttempts) {
        triggerBilingualNotification(`Deletion Error [PostgreSQL]: ${err.message}`);
        return { success: false, error: err.message };
      }
      await new Promise((resolve) => setTimeout(resolve, delay));
      delay *= 2;
