...
export const NursingConsole: React.FC<Props> = ({ patient, staffId, language = "ar" }) => {
  ...
  const saveVitals = async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent("openGenericModal", { detail: { titleEn: "Vital signs recorded", titleAr: "تم تسجيل العلامات الحيوية", type: "form" } }));
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {activeTab === "vitals" && (
          <div className="space-y-8">
            <div className="grid grid-cols-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* Temp */}
            ...
            <div className="pt-4 border-t border-slate-100 mt-8">
              <ClinicalSignaturePanel
                ...
                titleEn="Sign Vitals Record"
                titleAr="توقيع تسجيل المؤشرات الحيوية"
            ...
            <div className="p-6 bg-slate-900 text-white rounded-3xl shadow-xl flex flex-col gap-6">
               <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="text-lg sm:text-2xl font-black">+ 450 ml</h3>
                  </div>
               </div>
               <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
                 <ClinicalSignaturePanel
                    ...
                    onSave={() => toast.success(isAr ? "تم حفظ المسودة" : "Draft saved")}
                    onSign={() => {
                      toast.success(isAr ? "تم تسجيل ميزان السوائل بنجاح" : "I/O balance recorded successfully");
                    }}
                 />
               </div>
            </div>
