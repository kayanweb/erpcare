      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shadow-sm z-30">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="text-sm font-bold text-slate-400">{isAr ? "إدارة التحاليل والجودة" : "Lab Diagnostics & QA"}</span>
              <div className="w-1 h-1 bg-slate-300 rounded-full" />
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 bg-emerald-500 rounded-full" />
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">
                  {isAr ? "الأجهزة متصلة" : "Analyzers Online"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4">
           ...
           <button className="px-6 py-3 bg-indigo-600 text-white rounded-[20px] font-black uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center gap-2 active:scale-95">
             ...
             <span className="hidden lg:block">{isAr ? "مسح عينة" : "Scan Sample"}</span>
           </button>
        </div>
      </div>
            ...
            {mainTabs.map(tab => (
              ...
              >
                ...
                {activeMainTab === tab.id && (
                  <motion.div layoutId="lis-tab-active" className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600 rounded-t-full" />
                )}
              </button>
            ))}
         </div>
        ...
        <AnimatePresence mode="wait">
          {selectedSampleId ? (
            ...
            <div className="h-full overflow-y-auto no-scrollbar p-8">
                       ...
                       <div className="bg-slate-900 rounded-[40px] p-10 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl">
                          ...
                          <div>
                             <div className="flex justify-between items-start mb-10">
                                <h3 className="text-2xl font-black tracking-tight leading-tight uppercase">{isAr ? "تحكم جودة الأجهزة" : "Analyzer QC Monitor"}</h3>
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
                       <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                          <h2 className="text-2xl font-black text-slate-800 tracking-tight">{isAr ? "قائمة عمل المختبر" : "Clinical Work Queue"}</h2>
               ...
               {["samples", "phlebotomy", "inventory", "qc", "search"].includes(activeMainTab) && (
                 <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-8">
                    <div className="w-32 h-32 bg-indigo-50 rounded-[48px] flex items-center justify-center shadow-inner border border-indigo-100">
                       {mainTabs.find(t => t.id === activeMainTab)?.icon && React.createElement(mainTabs.find(t => t.id === activeMainTab)!.icon, { className: "w-16 h-16 text-indigo-300" })}
                    </div>
                    <div>
                       <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tight">{isAr ? "تجهيز النظام" : "Enterprise System Sync"}</h2>
                       <p className="text-slate-400 font-bold max-w-md mx-auto mt-4 leading-relaxed uppercase tracking-widest text-sm">
                         {isAr ? "يتم الآن ربط موديول " + activeMainTab + " ضمن نظام المختبرات الموحد" : "Linking " + activeMainTab + " module into the unified laboratory framework"}
                       </p>
                    </div>
