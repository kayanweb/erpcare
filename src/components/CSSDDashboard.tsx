...
export default function CSSDDashboard({ language }: { language: "ar" | "en" }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc]" dir={isAr ? "rtl" : "ltr"}>
      {/* Module Header */}
      <div className="bg-white border-b border-slate-200 px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm z-30">
        <div className="flex items-center gap-5">
           <div className="w-14 h-14 bg-indigo-900 rounded-[22px] flex items-center justify-center shadow-xl shadow-indigo-100 border-2 border-indigo-800 shrink-0">
...
function AssemblyWorkspace({ isAr }: { isAr: boolean }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
             ...
             <div className="grid grid-cols-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[1,2,3,4].map(i => (
                  <div key={i} className="aspect-square bg-slate-50 rounded-3xl border border-dashed border-slate-200 flex flex-col items-center justify-center p-6 text-center group hover:bg-white hover:border-indigo-300 transition-all">
                     <Layers className="w-8 h-8 text-slate-300 group-hover:text-indigo-500 mb-3" />
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {[
            ...
            <div key={load.id} className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm hover:shadow-xl transition-all group">
               ...
               <h4 className="text-2xl font-black text-slate-900 mb-1">{load.id}</h4>
