...
export const PatientClinicalHeader: React.FC<Props> = ({ patient, language, className = "", showVitals = true }) => {
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-2xl shadow-sm p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${className}`} dir={isAr ? "rtl" : "ltr"}>
      <div className="flex items-center gap-2 sm:gap-4 flex-wrap  w-full md:w-auto">
        <div className="relative group">
          <img 
            ...
            className="w-10 h-10 sm:w-14 sm:h-14 sm:w-16 sm:h-16 rounded-2xl shadow-sm border-2 border-white ring-1 ring-slate-100 object-cover" 
            alt="patient" 
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="flex items-center gap-1.5 bg-amber-50 px-2 py-1 rounded-lg border border-amber-100">
               ...
               <span className="text-amber-700 text-[10px] font-black uppercase tracking-tight">
                  {patient.currentClinicalLocation || (isAr ? "قيد الانتظار" : "PENDING LOCATION")}
               </span>
            </div>
          </div>
      ...
      {showVitals && (
        <div className="grid grid-cols-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 w-full md:w-auto">
          {[
