...
export default function AdmissionCenterDashboard({ language }: Props) {
  ...
  const { patients, updatePatient, updatePatientStatus, departments, beds, setBeds, currentUser, logAudit } = useHIS();
  ...
  const handleSignAdmission = (data: any, signedBy: string) => {
    if (showAssignModal && selectedBed) {
      const bed = beds.find(b => b.id === selectedBed);
      if (!bed) return;

      // Update patient location integrity
      updatePatient(showAssignModal.id, {
        status: 'ward',
        bedId: selectedBed,
        departmentId: bed.departmentId,
        roomId: bed.roomId,
        currentClinicalLocation: `${bed.building || 'Main'} - ${bed.floor || '1F'} - ${bed.roomNumber || 'Room'}`
      });

      // Update bed status globally
      setBeds(prev => prev.map(b => b.id === selectedBed ? { ...b, status: 'occupied', patientId: showAssignModal.id } : b));

      logAudit({
        action: 'ADMISSION_SIGNED',
        entityType: 'ADMISSION',
        entityId: showAssignModal.id,
        reason: `Bed ${bed.bedNumber} assigned in ${bed.departmentId}`,
        newValue: { bedId: selectedBed, signedBy }
      });

      toast.success(isAr ? "تم اعتماد التنويم والتوقيع بنجاح" : "Admission approved and signed successfully");
      setShowAssignModal(null);
      setSelectedBed("");
    } else {
      toast.error(isAr ? "يجب اختيار سرير أولاً" : "Must select a bed first");
    }
  };

  const stats = [
    { label: isAr ? "تنويم معلق" : "Pending Admissions", value: patients.filter(p => p.status === 'triage' || p.status === 'waiting').length.toString(), change: "+2", icon: Clock, color: "text-amber-600", bg: "bg-amber-100" },
  ...
  const pendingAdmissions = patients.filter(p => p.status === 'triage' || p.status === 'waiting');
                        ...
                        <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-600">
                              <span className="bg-slate-100 px-2 py-0.5 rounded border border-slate-200">ER</span>
                              <ArrowRightLeft size={10} className="text-slate-400" />
                              <span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded border border-indigo-100">{dept ? (isAr ? dept.nameAr : dept.nameEn) : "No Dept"}</span>
                            </div>
                          </td>
                          <td className="p-4">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase border bg-amber-50 text-amber-700 border-amber-100`}>
                              Urgent
                            </span>
                          </td>
                          <td className="p-4">
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isAr ? "الجناح المخصص" : "Assigned Department"}</label>
                  <div className="p-4 bg-slate-50 border-2 border-slate-100 rounded-xl font-bold text-slate-700">
                    {departments.find(d => d.id === showAssignModal.departmentId)?.[isAr ? 'nameAr' : 'nameEn'] || (isAr ? "غير محدد" : "Not Defined")}
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <select 
                    ...
                    className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-xl font-bold text-slate-700 focus:border-indigo-500 focus:ring-0 transition-all outline-none appearance-none"
                  >
                    <option value="">{isAr ? "اختر سريراً" : "Select Bed"}</option>
                    {beds
                      .filter(b => b.status === 'available' && b.departmentId === showAssignModal.departmentId)
                      .map(bed => (
                        <option key={bed.id} value={bed.id}>
                          {bed.bedNumber} {bed.roomNumber ? `(${bed.roomNumber})` : ""}
                        </option>
                      ))
                    }
                  </select>
                  {beds.filter(b => b.status === 'available' && b.departmentId === showAssignModal.departmentId).length === 0 && (
                    <p className="text-[10px] font-bold text-rose-500 mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> {isAr ? "لا توجد أسرة متاحة في هذا القسم" : "No beds available in this department"}
                    </p>
                  )}
                </div>
              </div>
