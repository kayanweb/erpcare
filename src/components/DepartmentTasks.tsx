...
export default function DepartmentTasks({ language, departmentId, departmentName, hideHeader = true }: Props & { hideHeader?: boolean }) {
  ...
  // Real or mock fallback active department patients
  ...
  useEffect(() => {
    ...
    // If context is empty, fall back to high-quality department-specific clinical mocks
    if (filtered.length === 0) {
      const defaultMocks: Record<string, Patient[]> = {
        "er": [
      ...
      filtered = defaultMocks[departmentId] || defaultMocks["dept-im"];
    }
              ...
              <div className="space-y-4">
                {dischargePatientsList.map((patient) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50 p-3.5 rounded-xl border border-slate-100 text-xs font-bold w-full lg:w-auto">
                      <div className="flex items-center gap-2">
