    ...
    <div className={`p-6 max-w-7xl mx-auto space-y-6 ${isAr ? "text-right" : "text-left"}`} dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 p-6 rounded-2xl shadow-xl text-white relative overflow-hidden">
        ...
        <div className="flex gap-4 relative z-10">
          <div className="bg-slate-800 border border-slate-700 p-3 rounded-xl flex items-center gap-4">
            <div className="flex flex-col">
              <span className="text-xs text-slate-400 uppercase tracking-wider">{isAr ? "المرضى تحت الخطر" : "Patients at Risk"}</span>
              <span className="text-2xl font-black text-rose-400">14</span>
            </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="space-y-4">
            {activeAlerts.map((alert) => (
              <div 
                key={alert.id} 
                className={`p-5 rounded-2xl border shadow-sm transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-4 ${getRiskBorder(alert.riskScore)}`}
              >
                <div className="flex items-start gap-4">
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center font-black text-lg shadow-lg shrink-0 ${getRiskColor(alert.riskScore)}`}>
                    {alert.riskScore}%
                  </div>
                  <div>
