...
export default function SmartAIAssistant({ language, currentUser, onClose }: any) {
  ...
  const handleSendChat = () => {
    ...
    setChatMessages([...chatMessages, 
      ...
      { role: 'bot', content: isAr ? 'شكراً لاستفسارك. يتم حالياً البحث في معايير الجودة والسياسات الخاصة بالمستشفى...' : 'Thank you for your inquiry. Searching hospital quality standards and policies...' }
    ]);
    setChatInput("");
  };
  ...
  const renderToolContent = () => {
    switch (activeTool) {
      ...
      case "med_interaction":
        return (
          <div className="p-4 space-y-4 text-right">
            ...
            <div className={`p-3 rounded text-sm font-bold border shadow-sm ${drug1 && drug2 ? 'bg-rose-50 border-rose-200 text-rose-700' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
              {drug1 && drug2 ? "⚠️ تحذير: قد يزيد خطر النزيف (Interaction Detected). المراقبة السريرية مطلوبة." : "أدخل الأدوية للفحص السريع"}
            </div>
          </div>
        );
      case "vital_analyzer":
      ...
      case "symptoms_ai":
        return (
          <div className="p-4 space-y-3 text-right">
             ...
             <textarea value={symptomInput} onChange={e=>setSymptomInput(e.target.value)} placeholder="وصف الحالة والأعراض... (مثال: ألم بالصدر مع ضيق تنفس)" className="w-full bg-white border border-slate-300 text-slate-900 rounded p-3 text-sm h-24 outline-none focus:ring-2 focus:ring-indigo-500 resize-none" />
             <div className="bg-indigo-50 p-2 text-[10px] text-indigo-700 rounded-lg border border-indigo-100 font-bold">
               {symptomInput.includes("صدر") ? "الاحتمالات: 1. ACS (اشتباه ذبحة)، 2. PE (جلطة رئوية)، 3. GERD" : "في انتظار وصف الأعراض..."}
             </div>
          </div>
        );
      case "lab_ranges":
      ...
      case "medical_dic":
        return (
          <div className="p-4 space-y-3 text-right">
             ...
             <input type="text" placeholder="بحث عن مصطلح... (مثال: Sepsis)" className="w-full bg-white border border-slate-300 text-slate-900 rounded p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
      ...
      case "policy_bot":
        return (
          <div className="flex-1 flex flex-col h-full min-h-0 text-right p-4">
             <div className="flex-1 space-y-3 overflow-y-auto mb-3 custom-scrollbar">
      ...
      case "shift_notes":
        return (
          <div className="p-4 space-y-3 text-right">
             ...
             <textarea 
               value={activeHandover} 
               onChange={e => setActiveHandover(e.target.value)} 
               placeholder="اكتب ملاحظاتك هنا (Situation, Background, Assessment, Recommendation)..." 
               className="w-full bg-white border border-slate-300 text-slate-900 rounded p-3 text-sm h-32 outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm resize-none" 
             />
             <div className="p-2 bg-indigo-50 border border-indigo-100 rounded-lg text-[10px] text-indigo-700 font-bold">
               {activeHandover ? "✅ تم تنسيق الملاحظات بصيغة SBAR جاهزة للطباعة." : "توليد SBAR آلي للمريض المختار."}
             </div>
