...
import { Plus, AppUser, UserRole } from "../types";
import { Plus, useHIS } from "../context/HISContext";
import { Plus, saveSystemUser } from "../lib/firestoreService";
import { Plus, 
  ShieldCheck, 
  UserPlus, 
  X, 
...
export default function UserApprovalDashboard({ 
  ...
  const { departments: hisDepartments, setDepartments: setHisDepartments } = useHIS();
  ...
  const getDeptsList = (): string[] => {
    if (hisDepartments && hisDepartments.length > 0) return hisDepartments.map(d => d.nameEn);
    try {
  ...
  return (
    <div className="space-y-6 text-right font-sans" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 text-white p-4 rounded-xl flex flex-col justify-between shadow-sm">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-lg sm:text-2xl font-black text-pink-500 font-mono">{pendingUsers.length}</span>
        ...
        <div className="bg-slate-900 border border-slate-800 text-white p-4 rounded-xl flex flex-col justify-between shadow-sm">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-lg sm:text-2xl font-black text-emerald-500 font-mono">{activeUsers.length}</span>
      ...
      {/* ========================================== */}
      {selectedUserForModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-dropdown flex items-center justify-center p-4">
          <div 
            ...
            <div className="p-6 overflow-y-auto max-h-[70vh] space-y-6 scrollbar-thin scrollbar-thumb-pink-200 scrollbar-track-slate-50">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
                    ...
                    <div className="space-y-2.5 text-xs text-right">
                      ...
                      {/* Dynamic department/clinic selection dropdown */}
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <label className="block text-[10px] font-black text-rose-900">القسم والعيادة المعتمدة (من جدول الوحدات) *:</label>
                          <button
                            type="button"
                            onClick={() => {
                              const newDept = prompt("Enter new department name / أدخل اسم القسم الجديد:");
                              if (newDept && newDept.trim()) {
                                setHisDepartments([...hisDepartments, {
                                  id: `dept-${Date.now()}`,
                                  nameEn: newDept.trim(),
                                  nameAr: newDept.trim(),
                                  type: 'clinical'
                                }]);
                                setModalForm({ ...modalForm, department: newDept.trim() });
                              }
                            }}
                            className="text-[9px] text-pink-600 hover:text-pink-800 font-bold flex items-center gap-1"
                          >
                            <Plus size={10} /> إضافة قسم جديد
                          </button>
                        </div>
                        <select
                      ...
                      {SYSTEM_MODULES.map((mod) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 text-[9px] font-bold">
                              <button
                                type="button"
                ...
                <div className="lg:col-span-7 bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-4">
                  ...
                  <div className="grid grid-cols-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1.5 text-[9px] font-bold">
                    <button
                      type="button"
