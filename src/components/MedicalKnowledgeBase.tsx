...
export const MedicalKnowledgeBase: React.FC<Props> = ({ language }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const records = await firestoreService.getAll<DataRecord>('medicalknowledgebase');
        setData(records);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className={`p-6 max-w-7xl mx-auto space-y-6 ${isAr ? 'text-right' : 'text-left'}`} dir={isAr ? 'rtl' : 'ltr'}>
      <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">
        {isAr ? "MedicalKnowledgeBase" : "Medical Knowledge Base"}
      </h2>
      {loading ? (
