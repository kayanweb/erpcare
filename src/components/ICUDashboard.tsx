    ...
    <div className="flex flex-col h-full bg-[#f8fafc]" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="bg-slate-900 text-white px-8 py-6 flex items-center justify-between shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          <div className="w-14 h-14 bg-indigo-600 rounded-[24px] flex items-center justify-center shadow-lg shadow-indigo-500/20 border border-white/10 animate-pulse">
            ...
            {mainTabs.map(tab => (
              ...
              >
                ...
                {activeMainTab === tab.id && (
                  <motion.div layoutId="icu-tab-active" className="absolute bottom-0 left-0 w-full h-1 bg-indigo-600 rounded-t-full" />
                )}
              </button>
            ))}
         </div>
            ...
            <>
              {activeMainTab === "dashboard" && (
                <motion.div 
                   ...
                   initial={{ opacity: 0 }}
                   animate={{ opacity: 1 }}
                   className="p-8 h-full overflow-y-auto no-scrollbar space-y-8"
                >
                          ...
                          <div key={bedNum} className="bg-white rounded-[32px] border border-slate-200 p-6 shadow-sm hover:shadow-xl transition-all cursor-pointer group flex flex-col gap-4 relative overflow-hidden" onClick={() => setSelectedPatientId(patient?.id || null)}>
                             ...
                             <div className="flex justify-between items-start">
                                ...
                                <div className="text-right">
                                   ...
                                   <span className="text-2xl font-black text-slate-900 tracking-tighter">ICU-0{bedNum}</span>
                                </div>
                             </div>
              ...
              {["patients", "vitals", "ventilators", "tasks", "search", "analytics"].includes(activeMainTab) && (
                <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-8">
                   ...
                   <div>
                      <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tight">{isAr ? "قيد المزامنة" : "Enterprise Unit Sync"}</h2>
                      <p className="text-slate-400 font-bold max-w-md mx-auto mt-4 leading-relaxed uppercase tracking-widest text-sm">
                        {isAr ? "يتم الآن دمج بيانات " + activeMainTab + " ضمن نظام العناية الموحد" : "Integrating " + activeMainTab + " data layer into the unified critical care framework"}
                      </p>
                   </div>
                   <div className="flex gap-2">
