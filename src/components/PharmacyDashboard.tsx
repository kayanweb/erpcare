...
import { 
  ...
  Search, Filter, History, Package, Truck, Beaker, Clock, Plus
} from "lucide-react";
...
export default function PharmacyDashboard({ language }: { language: "ar" | "en" }) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-h-0 bg-[#f8fafc]" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="bg-white border-b border-slate-200 px-4 sm:px-8 py-4 sm:py-5 flex flex-col lg:flex-row lg:items-center justify-between shadow-sm z-30 gap-6 shrink-0">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 sm:gap-5">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <h1 className="text-xl sm:text-lg sm:text-2xl font-black text-slate-900 tracking-tight truncate">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 sm:gap-4 w-full lg:w-auto">
      ...
      {/* Navigation Tabs */}
      <div className="bg-white border-b border-slate-200 overflow-x-auto no-scrollbar sticky top-0 z-20 shrink-0">
            ...
            {mainTabs.map(tab => (
              ...
              >
                ...
                {activeMainTab === tab.id && (
                  <motion.div  className="absolute bottom-0 left-0 w-full h-1 bg-emerald-600 rounded-t-full" />
                )}
              </button>
            ))}
         </div>
          ...
          {selectedRxId ? (
             ...
             >
                ...
                <div className="flex-1 p-8 overflow-y-auto no-scrollbar space-y-8">
                   ...
                   <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-sm">
                      <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6">{isAr ? "المصادقة على الصرف" : "Dispensing Authentication"}</h4>
                      <ClinicalSignaturePanel
                        language={language}
                        currentUser={currentUser}
                        titleEn="Verify & Sign Medication Release"
                        titleAr="المصادقة والتوقيع على صرف الدواء"
                        onSave={() => toast.success(isAr ? "تم حفظ المسودة" : "Draft saved")}
                        onSign={() => {
                           toast.success(isAr ? "تم اعتماد وصرف الدواء بنجاح" : "Medication verified and dispensed");
                           setSelectedRxId(null);
                        }}
                      />
                   </div>
            ...
            <div className="h-full overflow-y-auto no-scrollbar p-3 sm:p-6 lg:p-8">
                       ...
                       <div className="bg-slate-900 rounded-[40px] p-10 text-white relative overflow-hidden flex flex-col justify-between shadow-2xl border border-slate-800">
                          ...
                          <div>
                             <div className="flex justify-between items-start mb-10">
                                <h3 className="text-lg sm:text-2xl font-black tracking-tight leading-tight uppercase">{isAr ? "تحكم المخزون الذكي" : "Smart Stock Control"}</h3>
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
                       <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                          <h2 className="text-lg sm:text-2xl font-black text-slate-800 tracking-tight">{isAr ? "طابور الصرف الموحد" : "Unified Dispensing Worklist"}</h2>
                                  ...
                                  return (
                                    <tr key={rx.id} className="group hover:bg-slate-50/50 transition-all">
                                       ...
                                       <td className="py-5 px-8">
                                          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                             <div className="w-5 h-5 sm:w-8 sm:h-8 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400 font-black text-xs uppercase">{patient ? patient.nameEn[0] : "?"}</div>
                                             <div>
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
                       <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                          <div>
                             <h2 className="text-lg sm:text-2xl font-black text-slate-800 tracking-tight">{isAr ? "مخزون الأدوية والمستلزمات" : "Medication & Supply Inventory"}</h2>
                             <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{isAr ? "مراقبة مستويات المخزون والباركود" : "Stock level monitoring & Barcode control"}</p>
                          </div>
                          <div className="flex gap-4">
                             <button className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100">
                                <Plus className="w-4 h-4" />
                                <span>{isAr ? "طلب شراء" : "New PO"}</span>
                             </button>
                          </div>
                       </div>
                       <div className="overflow-x-auto">
                          <table className="w-full text-left">
                             <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                <tr>
                                   <th className="py-5 px-8">{isAr ? "الاسم التجاري" : "Trade Name"}</th>
                                   <th className="py-5 px-8">{isAr ? "الشكل الدوائي" : "Form"}</th>
                                   <th className="py-5 px-8">{isAr ? "الكمية" : "Qty"}</th>
                             ...
                             <tbody className="divide-y divide-slate-50">
                                {[
                                   { name: "Augmentin 1g", form: "Tablet", qty: "1,240", expiry: "2026-05", status: "Healthy" },
                                   { name: "Lantus SoloStar", form: "Injection", qty: "12", expiry: "2025-09", status: "Low Stock" },
                                   { name: "Nexium 40mg", form: "Capsule", qty: "850", expiry: "2026-01", status: "Healthy" },
                                   { name: "Ventolin Evohaler", form: "Inhaler", qty: "0", expiry: "N/A", status: "Stock Out" },
                                ].map((item, i) => (
                                   <tr key={i} className="group hover:bg-slate-50/50 transition-all">
                                      <td className="py-5 px-8">
                                         <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                            <div className="w-5 h-5 sm:w-8 sm:h-8 bg-emerald-100 rounded-lg flex items-center justify-center text-emerald-600">
                                               <Package className="w-4 h-4" />
                                            </div>
                                            <span className="text-sm font-black text-slate-800">{item.name}</span>
                                         </div>
                                      </td>
                                      <td className="py-5 px-8 text-xs font-bold text-slate-500 uppercase">{item.form}</td>
                                      <td className="py-5 px-8 text-sm font-black text-slate-700">{item.qty}</td>
               ...
               {activeMainTab === "iv-prep" && (
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-10">
                       <div className="flex justify-between items-center mb-10">
                          <div>
                             <h3 className="text-xl font-black text-slate-900 tracking-tight">{isAr ? "وحدة التحضير الوريدي (IV)" : "IV Admixture Unit"}</h3>
                             <p className="text-sm font-bold text-slate-400 mt-1 uppercase tracking-widest">{isAr ? "تحضير المحاليل والعلاجات الكيماوية" : "Sterile Compounding & Chemotherapy Prep"}</p>
                          </div>
                          <div className="flex gap-4">
                             <span className="px-4 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-black border border-emerald-100">3 Active Preps</span>
                          </div>
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {[
                             { patient: patients[0], medication: "Normal Saline + KCl", volume: "500ml", status: "Compounding", tech: "Pharmacist Nora" },
                             { patient: patients[2], medication: "Vancomycin 1g", volume: "250ml", status: "QC Check", tech: "Pharmacist Zeyad" },
                          ].map((p, i) => (
                            <div key={i} className="p-8 bg-slate-50 rounded-[32px] border border-slate-100 hover:bg-white hover:shadow-xl hover:border-emerald-100 transition-all group">
                               <div className="flex justify-between items-start mb-6">
                                  <div className="w-12 h-12 bg-emerald-100 rounded-2xl flex items-center justify-center">
                                     <Beaker className="w-6 h-6 text-emerald-600" />
                                  </div>
                                  <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${p.status === 'QC Check' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                                     {p.status}
                                  </span>
                               </div>
                               <h4 className="font-black text-slate-800 text-lg leading-tight mb-2">
                               ...
                               <p className="text-sm font-bold text-emerald-600 mb-6">{p.medication} • {p.volume}</p>
                               <div className="flex justify-between items-center pt-6 border-t border-slate-100">
                                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Assigned: {p.tech}</span>
                                  <button className="px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all">
               ...
               {activeMainTab === "clinical" && (
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-10">
                       <div className="flex justify-between items-center mb-10">
                          <div>
                             <h3 className="text-xl font-black text-slate-900 tracking-tight">{isAr ? "الصيدلة السريرية والتدخلات" : "Clinical Pharmacy & Interventions"}</h3>
                             <p className="text-sm font-bold text-slate-400 mt-1 uppercase tracking-widest">{isAr ? "مراقبة سلامة الدواء وتعديل الجرعات" : "Monitoring medication safety & dose adjustments"}</p>
                          </div>
                          <div className="flex gap-4">
                             <button className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-emerald-100">
                                {isAr ? "سجل تدخل جديد" : "New Intervention"}
                             </button>
                          </div>
                       </div>
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="space-y-4">
                             <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{isAr ? "تنبيهات عالية الخطورة" : "High Risk Alerts"}</h4>
                             {[
                                { patient: patients[0], alert: "Dose Adjustment Required", reason: "Renal Impairment (CrCl < 30)", drug: "Enoxaparin" },
                                { patient: patients[1], alert: "Potential Interaction", reason: "Warfarin + Amiodarone", drug: "Warfarin" },
                             ].map((a, i) => (
                                <div key={i} className="p-6 bg-rose-50 border border-rose-100 rounded-[32px] flex flex-col justify-between group hover:bg-rose-100 transition-all">
                                   <div className="flex justify-between items-start mb-4">
                                      <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-rose-200">
                                         <AlertTriangle className="w-5 h-5 text-rose-600" />
                                      </div>
                                      <span className="text-[9px] font-black px-2 py-1 bg-rose-600 text-white rounded-lg uppercase tracking-widest">Urgent</span>
                                   </div>
                                   <h5 className="font-black text-rose-900 text-sm mb-1">{isAr ? a.patient.nameAr : a.patient.nameEn}</h5>
                                   <p className="text-xs font-bold text-rose-700 mb-4">{a.alert}: {a.drug}</p>
                                   <div className="p-3 bg-white/50 rounded-xl text-[10px] font-bold text-rose-800">
                                      {a.reason}
                                   </div>
                                </div>
                             ))}
                          ...
                          <div className="bg-slate-50 rounded-[32px] border border-slate-100 p-8">
                             <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">{isAr ? "إحصائيات التدخلات السريرية" : "Clinical Intervention Stats"}</h4>
                             <div className="space-y-6">
                                {[
                                   { label: "Dose Optimization", val: "42", color: "emerald" },
                                   { label: "Drug Interactions", val: "18", color: "amber" },
                                   { label: "Cost Saving Advice", val: "24", color: "blue" },
                                ].map((s, i) => (
                                   <div key={i} className="flex items-center justify-between">
                                      <span className="text-sm font-bold text-slate-600">{s.label}</span>
                                      <span className={`px-4 py-1 bg-${s.color}-100 text-${s.color}-700 rounded-xl font-black text-xs`}>{s.val}</span>
                                   </div>
                                ))}
                             </div>
                             <div className="mt-10 p-6 bg-white border border-slate-200 rounded-3xl text-center">
                                <Activity className="w-5 h-5 sm:w-8 sm:h-8 text-emerald-500 mx-auto mb-3" />
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{isAr ? "إجمالي المساهمات السريرية" : "Total Clinical Value"}</p>
                                <p className="text-lg sm:text-2xl font-black text-slate-800">$12,450</p>
                             </div>
                          </div>
                       </div>
                    </div>
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-10">
                       <div className="max-w-2xl mx-auto text-center mb-12">
                          <h3 className="text-lg sm:text-2xl font-black text-slate-900 tracking-tight mb-4">{isAr ? "أرشيف الوصفات الدوائية" : "Prescription History Archive"}</h3>
                          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                             {isAr ? "البحث في تاريخ الوصفات، سجلات الصرف، وتقارير مراقبة المخزون" : "Search prescription history, dispensing logs, and inventory audit reports"}
                          </p>
                       </div>
                       <div className="max-w-3xl mx-auto flex flex-col gap-8">
                          <div className="relative">
                             <Search className="w-6 h-6 absolute left-6 top-1/2 -translate-y-1/2 text-emerald-500" />
                             <input 
                                type="text" 
                                placeholder={isAr ? "ابحث برقم الوصفة، اسم المريض، أو الدواء..." : "Search by Rx ID, Patient name, or Medication..."} 
                                className="w-full pl-16 pr-8 py-6 bg-slate-50 border-2 border-slate-100 rounded-[32px] text-lg font-bold outline-none focus:border-emerald-500/30 focus:bg-white transition-all shadow-inner"
                             />
                             <button className="absolute right-4 top-1/2 -translate-y-1/2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all">
                                {isAr ? "بحث تاريخي" : "Search History"}
                             </button>
                          </div>
                          <div className="grid grid-cols-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                             {[
                                { label: isAr ? "وصفات العيادات" : "Outpatient Rx", icon: Pill },
                                { label: isAr ? "أدوية التنويم" : "Inpatient Meds", icon: Box },
                                { label: isAr ? "محاليل وريدية" : "IV Archives", icon: Beaker },
                                { label: isAr ? "سجلات المخزون" : "Inventory Logs", icon: History },
                             ].map((cat, i) => (
                                <button key={i} className="p-6 bg-white border border-slate-200 rounded-3xl hover:border-emerald-500/50 hover:shadow-xl transition-all flex flex-col items-center gap-3 group">
                                   <cat.icon className="w-5 h-5 sm:w-8 sm:h-8 text-slate-300 group-hover:text-emerald-600 transition-colors" />
                                   <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-emerald-600">{cat.label}</span>
                                </button>
                             ))}
                          </div>
                       </div>
