...
export function HISProvider({ children, isLoggedIn, language = "ar", currentUser = null }: { children: ReactNode, isLoggedIn: boolean, language?: "ar" | "en", currentUser?: any }) {
  ...
  const systemUsers = Array.isArray(systemUsersRaw) ? systemUsersRaw : EMPTY_ARRAY;
  const dutyTasks = Array.isArray(dutyTasksRaw) ? dutyTasksRaw : EMPTY_ARRAY;
  const clinicalRecords = Array.isArray(clinicalRecordsRaw) ? clinicalRecordsRaw : EMPTY_ARRAY;
